...
def login(config, signer, auth_mode):
    if auth_mode == 'instance_principal' or auth_mode == 'config_file':
        identity = oci.identity.IdentityClient(config, signer=signer)
        user = identity.get_user(config['user']).data
        #print("Logged in as: {} @ {}".format(user.description, config['region']))
    else:
        identity = oci.identity.IdentityClient(config, signer=signer)
        #print("Logged in with { Delegation Token } in { CloudShell }")

def get_compartment_list(config, signer, tenancy_id):
    identity = oci.identity.IdentityClient(config, signer=signer)

    target_compartments = []
    all_compartments = []

    top_level_compartment_response = identity.get_compartment(tenancy_id)
